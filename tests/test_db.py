...
def _wait_db_task(cur, task_id, expected_state, timeout=5.0):
    ...
    while time.monotonic() < deadline:
        cur.execute("SELECT state, finished_at, error FROM tasks WHERE id = %s", [task_id])
        row = cur.fetchone()
        if row and row[0] == expected_state:
            return row
        time.sleep(0.1)
    raise TimeoutError(f"Task {task_id} not in DB state '{expected_state}' after {timeout}s")


def _wait_db_session_opened(cur, session_id, timeout=5.0):
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        cur.execute("SELECT opened_at, closed_at FROM sessions WHERE id = %s", [session_id])
        row = cur.fetchone()
        if row and row[0] is not None:
            return row
        time.sleep(0.1)
    raise TimeoutError(f"Session {session_id} 'opened_at' not set in DB after {timeout}s")
...
def test_session_inserted_in_db(qscheduler_test, db_connect):
    ...
    with db_connect.cursor() as cur:
        cur.execute(
            "SELECT id, machine_id, time_limit_secs FROM sessions WHERE id = %s",
            [session_id],
        )
        row = cur.fetchone()
...
def test_session_closed_with_cancelled_tasks_in_db(qscheduler_test, db_connect):
    ...
    qscheduler_test.wait_for_session_open(session_id)
    task_id = qscheduler_test.submit(TT().submit_time(3).compute_time(1), session_id=session_id)
